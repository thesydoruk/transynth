 ...
 * navigates to /games/:gameId/diff?newModId=X&oldModId=Y which auto-triggers the comparison.
 *
...
export const ReimportModal = ({ newModId, prevVersions, onClose }: ReimportModalProps) => {
  ...
  const openDiff = () => {
    if (selectedId == null || !routeGameId) return;
    onClose();
    navigate(`/games/${routeGameId}/diff?newModId=${newModId}&oldModId=${selectedId}`);
  };
  ...
  return (
    <ModalShell onClose={onClose} customHeader={header} hideCloseButton>
      ...
      <div className={s.versionList}>
        {prevVersions.map((v) => {
          const pct =
            v.total_strings > 0 ? Math.round((v.translated_strings / v.total_strings) * 100) : 0;
