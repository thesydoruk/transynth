...
const isVoiceCancelError = (err: unknown): boolean => {
  if (!(err instanceof Error)) return false;
  return /cancelled by user/i.test(err.message);
};

const isVoiceStopHeld = (modId: number): boolean => {
  const status = getModAiJob(modId, 'voice').status;
  return status === 'stopping' || status === 'cancelled';
};
...
export const startModAiVoice = async (
  ...
  try {
    ...
    const snapshot = await api.voiceGenerate.start(
      ...
      (event) => {
        if (event.type === 'started') {
          ...
          upsertModAiJob(modId, 'voice', {
            status: isVoiceStopHeld(modId) ? 'stopping' : 'running',
            jobId: event.jobId,
            total: event.total,
            done: 0,
            speakerKey: scopedSpeaker,
        ...
        if (event.type === 'progress') {
          upsertModAiJob(modId, 'voice', {
            status: isVoiceStopHeld(modId) ? getModAiJob(modId, 'voice').status : 'running',
            done: event.done,
            total: event.total,
            speakerKey: scopedSpeaker,
          });
    ...
    if (isVoiceCancelError(err) || isVoiceStopHeld(modId)) {
      upsertModAiJob(modId, 'voice', {
        status: 'cancelled',
        speakerKey: scopedSpeaker,
        error: null,
