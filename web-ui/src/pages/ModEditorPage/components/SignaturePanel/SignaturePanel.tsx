...
export interface SignaturePanelProps {
  ...
  /** Total number of strings matching the current query. */
 ...
 */
export const SignaturePanel = ({ sigCounts, activeSignature, totalFiltered, onSelect }: SignaturePanelProps) => {
  const { t } = useTranslation();
  const totalAll = sigCounts.reduce((a, r) => a + Number(r.count), 0);
  ...
  return (
    <div className={styles.leftPanel}>
      ...
      >
        ...
        <span className={styles.sigCount}>{totalFiltered ?? '…'} / {totalAll}</span>
      </div>
      {sigCounts.map((sig) => (
        <div
          key={sig.signature}
