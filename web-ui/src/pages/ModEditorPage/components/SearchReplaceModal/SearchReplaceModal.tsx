...
export const SearchReplaceModal = ({
  ...
  const [previewResult, setPreviewResult] = useState<{
    matches: Array<{ originalText: string; newText: string; formid_hex: string }>;
    applied: number;
  } | null>(null);
  ...
  const handlePreview = async () => {
    ...
    setError(null);
    try {
      const result = await api.search.replace(modId, {
        search,
        replace,
        isRegex,
        targetLang,
  ...
  return (
    <ModalShell
      title={t('modEditor.searchReplaceTitle')}
      onClose={onClose}
      closeAriaLabel={t('common.close')}
    >
      <div className={s.modalForm}>
        <input
          placeholder={t('modEditor.searchLabel')}
          value={search}
          onChange={(event) => setSearch(event.target.value)}
          className={s.modalInput}
        />
        <input
          placeholder={t('modEditor.replaceLabel')}
          value={replace}
        ...
        <label className={s.modalRegexLbl}>
          ...
          />{' '}
          {t('modEditor.useRegex')}
        </label>
      </div>
      <div className={s.modalBtnRow}>
        <Button variant="neutral" onClick={handlePreview} disabled={stage !== 'idle' || !search}>
          {t('modEditor.preview', { count: previewResult?.matches.length ?? 0 })}
        </Button>
        <Button variant="primary" onClick={handleApply} disabled={stage !== 'idle' || !search}>
          {t('common.apply')}
        </Button>
      ...
      {stage === 'done' && (
        <p className={s.modalOk}>{t('modEditor.applied', { count: previewResult?.applied })}</p>
      )}
      {previewResult && stage !== 'done' && previewResult.matches.length > 0 && (
        <div className={s.modalPreview}>
          ...
          {previewResult.matches.length > 20 && (
            <p className={s.modalPrevMore}>
              {t('modEditor.more', { count: previewResult.matches.length - 20 })}
            </p>
          )}
        </div>
      )}
