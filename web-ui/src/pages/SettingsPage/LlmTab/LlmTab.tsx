...
export const LlmTab = () => {
  ...
  const savedServers: VllmServerDraft[] = (projectSettings?.['llm.vllm_servers'] ?? []).map(
    (server) => ({
      host: server.host,
  ...
  return (
    <>
      <div className={s.readonlyNote}>{t('settings.llm.readonlyNote')}</div>
