  ...
  return (
    <section className={s.section}>
      ...
      <div className={s.cards}>
        ...
        <MiniCard label={t('ops.memory')} value={fmtBytes(system.heapUsedBytes)} sub={`/ ${fmtBytes(system.heapTotalBytes)}`} />
        <MiniCard label={t('ops.rss')} value={fmtBytes(system.memoryRssBytes)} />
        <MiniCard label={t('ops.dbStatus')} value={system.dbConnected ? t('ops.dbOk') : t('ops.dbDown')} color={system.dbConnected ? '#4caf50' : '#e55'} />
      </div>
    </section>
  );
};
