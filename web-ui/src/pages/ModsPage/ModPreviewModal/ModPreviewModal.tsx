...
import { api, type ModImportJob, type ModPreviewRow } from '../../../api';
...
/** Preview modal for mod imports — review strings, then import all available content. */
export const ModPreviewModal = ({ job, onClose, onConfirm }: ModPreviewModalProps) => {
  ...
  return (
    <ModalShell onClose={onClose} customHeader={header} hideCloseButton size="xl" stretchContent>
      <p className={s.importHint}>{t('modImport.importAllHint')}</p>
      ...
      <div className={parentS.footer}>
        ...
        <Button variant="success" onClick={onConfirm}>
          {t('modImport.importAllAction', { count: job.total_records.toLocaleString() })}
        </Button>
      </div>
    </ModalShell>
  );
