...
export const INNRPage = () => {
  ...
  const handleClear = useCallback((stringId: number) => clearMut.mutate({ stringId }), [clearMut]);

  // ── Derived stats ─────────────────────────────────────────────────────────
  const totalGroups = data?.groups.length ?? 0;
  const totalRows = data?.total_rows ?? 0;
  const translatedRows =
    data?.groups.flatMap((g) => g.rows).filter((r) => r.translation !== null).length ?? 0;

  /** Groups filtered by search query on base EDID. */
  const filteredGroups = search
    ? (data?.groups ?? []).filter((g) => g.base_edid.toLowerCase().includes(search.toLowerCase()))
    : (data?.groups ?? []);
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.header}>
        ...
        {data?.mod_name && <span className={s.modName}>{data.mod_name}</span>}
      </div>
      ...
      {!isLoading && totalRows === 0 && <div className={s.empty}>{t('innr.noRecords')}</div>}
