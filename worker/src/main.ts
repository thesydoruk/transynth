 ...
 *
 * Flow:
 *   1. BullMQ claims a job from Redis (`transynth-jobs` / `-llm` / `-voice`).
...
const startQueueWorker = (
  ...
  const worker = new Worker<JobData>(queueName, (job) => processJob(db, job), {
    connection: createRedisConnection(role),
    concurrency,
    ...
    // voice-generate is re-queued as missing without resetVoice.
    lockDuration: 300_000,
    maxStalledCount: 0,
  });
  worker.on('error', (err) => logJobs.warn(`worker error (${queueName}): ${err.message}`));
  worker.on('failed', (job, err) => {
    logJobs.error('job failed', {
      queue: queueName,
      jobId: job?.id,
      kind: job?.data.kind,
      error: err.message,
...
const workers = [
  startQueueWorker(JOBS_QUEUE_NAME, 'worker', CONFIG.jobConcurrency),
  startQueueWorker(LLM_QUEUE_NAME, 'llm-worker', 1),
  startQueueWorker(VOICE_QUEUE_NAME, 'voice-worker', 1),
];
...
logJobs.info(`Job workers started (jobs=${CONFIG.jobConcurrency}, llm=1, voice=1)`);
