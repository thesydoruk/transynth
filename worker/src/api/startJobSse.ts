...
import { allocateJobId, enqueueJob, queueNameForKind, toBullJobId } from '../core/queue';
import { getQueueEvents } from '../core/queueEvents';
...
export const relayJobToSse = (jobId: number, stream: SseStream, kind: JobKind): void => {
  ...
  const onFailed = (args: { jobId: string; failedReason: string }): void => {
    if (args.jobId !== id) return;
    const reason = args.failedReason || 'Job failed';
    if (/cancelled by user/i.test(reason)) {
      stream.send({ type: 'cancelled', done: 0, total: 0 });
    } else {
      stream.send({ type: 'error', error: reason });
    }
    finish();
  };
...
export const startJobSse = async (
  ...
  relayJobToSse(jobId, stream, opts.data.kind);
