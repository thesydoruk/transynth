...
describe('recoverOrphanedVoiceGenerateJobs', () => {
  it('re-enqueues active voice-generate and fails the old running snapshot', async () => {
    ...
    expect(deps.enqueueJob).toHaveBeenCalledWith(
      {
        ...voiceData(),
        params: { ...voiceData().params, scope: 'missing', resetVoice: false },
      },
      800,
    );
    expect(deps.writeSnapshot).toHaveBeenCalledWith(
  ...
  it('resumes a regenerate-all job as missing without wiping again', async () => {
    const job = makeJob('job-723', {
      kind: 'voice-generate',
      modId: 33,
      params: { srcLang: 'en', targetLang: 'uk', scope: 'all', resetVoice: true },
    });
    ...
    expect(deps.enqueueJob).toHaveBeenCalledWith(
      {
        kind: 'voice-generate',
        modId: 33,
        params: { srcLang: 'en', targetLang: 'uk', scope: 'missing', resetVoice: false },
      },
      800,
    );
  });
