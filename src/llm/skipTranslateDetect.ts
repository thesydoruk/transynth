...
export const buildSkipDetectSystemPrompt = (
  ...
  return [
    ...
    '- Every input row already passed a fast heuristic scan. Obvious non-translatable rows were removed upstream: empty/placeholder-only text, markup-only strings, ARMA records, FormID-like hex, file paths, MPS/LightNode internal names, pure numbers/symbols, editor-ID duplicates, short stat codes, and many PEX debug literals.',
    ...
    '- context = speaker / scene hint; for PEX, often decompiled .psc lines around the literal; for MCM, page=/type=/help=/label= of the paired menu control.',
    '- Player-facing: INFO/DIAL (dialogue), BOOK, WEAP/ARMO FULL (items), quest stages, most UI strings, MCM menu labels.',
    '',
    ...
    '- Any readable words meant for the player: dialogue, item/weapon/armor names, descriptions, books, notes, quest text, UI labels, MCM options.',
    '- Proper names, location names, character names — even on ACTI or unusual record types.',
    '- INNR (grup = INNR, field = WNAM) — bare name fragments the game concatenates into item names ("Violent", "Junkie\'s", "Insulated", "Sniper Rifle"). They look contextless on their own but the player reads them.',
