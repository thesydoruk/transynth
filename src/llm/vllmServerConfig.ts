...
/** Chat-pool hosts must be http(s). The server will call them (operator-trusted LAN). */
export const isAllowedLlmEndpointUrl = (raw: string): boolean => {
  try {
...
export const normalizeVllmServerEntries = (raw: unknown): VllmServerEntry[] => {
  ...
  for (const item of raw) {
    ...
    if (!isAllowedLlmEndpointUrl(host)) {
      log.warn(`vLLM server entry skipped: host must be http(s) (${host})`);
      continue;
    }
    servers.push({
      host,
...
/** Public pool row: never includes the raw key. */
export type VllmServerPublic = {
  host: string;
  maxParallel: number;
  apiKeyConfigured: boolean;
};

export const redactVllmServerEntries = (servers: readonly VllmServerEntry[]): VllmServerPublic[] =>
  servers.map(({ host, maxParallel, apiKey }) => ({
    host,
    maxParallel,
    apiKeyConfigured: Boolean(apiKey),
  }));

/** Empty incoming `apiKey` keeps the stored key for the same host. */
export const retainVllmApiKeys = (
  incoming: readonly VllmServerEntry[],
  stored: readonly VllmServerEntry[],
): VllmServerEntry[] =>
  incoming.map((server) => ({
    ...server,
    apiKey: server.apiKey.trim() || stored.find((row) => row.host === server.host)?.apiKey || '',
  }));
