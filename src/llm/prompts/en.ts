...
export const buildEnglishTranslateSystemPrompt = (
  ...
  return [
    ...
    '- Translate every word, including category and subcategory labels. Do not leave English prefixes or standalone English words.',
    '- Pattern "[Category] - [Subcategory]" → fully translated in the target language for BOTH parts.',
    '- Do not mix languages (e.g. keep "Ammo" or "Armor" in English while translating only the second half).',
    '',
    '### CAPITALIZATION:',
    ...
    '- Title case or sentence case in the source ("Wood", "Bamboo Fiber") → natural capitalization in the target language, not all-caps styling.',
    '- Use ALL CAPS only when the source is entirely upper-case (e.g. short UI abbreviations like "HP", "AP").',
...
export const buildEnglishVerifySystemPrompt = (
  ...
  return [
    ...
    '- Capitalization: if the source is not ALL CAPS but the translation uses gratuitous ALL CAPS (e.g. "Wood" → "WOOD" equivalent), classify as "suspicious" with a corrected "suggestion".',
    '- UI category labels: English residue in a mixed label ("Ammo — ballistic" with "Ammo" untranslated) — "incorrect"; both parts must be in the target language.',
