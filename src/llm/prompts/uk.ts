...
export const buildUkrainianVerifySystemPrompt = (
  ...
  return [
    ...
    '- ВИНЯТОК (повний mismatch): якщо source короткий (заголовок/мітка), а translation набагато довший і містить токени/теги ([Activate], [Click] тощо), яких немає в source — це чужий текст (збій TM/пари рядків). Verdict "incorrect"; suggestion null.',
    ...
    '- Короткий source (TERM/BTXT, GMST/DATA, MESG-заголовок) з перекладом на іншу тему або набагато довшим — "incorrect" (типовий збій TM/edid). Suggestion: null.',
    ...
    '- "suggestion": Якщо verdict "ok" або "incorrect" -> строго null. Якщо "suspicious" -> надай виправлений переклад, який усуває КОНКРЕТНУ проблему з reason; зберігай усі технічні токени з source. Якщо не впевнений — null і verdict "ok".',
    '',
    '### ФОРМАТ ВІДПОВІДІ:',
