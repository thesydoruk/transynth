...
export const buildUkrainianVerifySystemPrompt = (
  ...
  return [
    ...
    '- Два варіанти з однаковим змістом (стислий vs розлогий, «Низ вуха» vs «Нижня частина вуха») — verdict "ok"; не пропонуй перефраз лише за стилем.',
    ...
    '- Короткий source (TERM/BTXT, GMST/DATA) з перекладом на іншу тему або набагато довшим — "incorrect" (типовий збій TM/edid).',
    '- GMST/DIAL/MESG: короткі фіксовані варіанти меню (Barter, Not Interested, Sarcastic) — не "suspicious" за синонімами, якщо зміст OK.',
    '- Якщо переклад прийнятний, але твій suggestion відхилено б guard-логікою — verdict "ok", не "suspicious".',
