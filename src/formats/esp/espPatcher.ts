...
const rebuildRange = (buf: Buffer, start: number, end: number, patchMap: PatchMap): Buffer => {
  ...
  while (pos + RECORD_HEADER_SIZE <= end) {
    ...
    if (sig === 'GRUP') {
      ...
      // A size smaller than the GRUP header cannot advance the walk; copy the
      // rest verbatim instead of looping forever / dropping data.
      if (groupSize < GRUP_HEADER_SIZE) {
        log.warn(`ESP patcher: corrupt GRUP size ${groupSize} at offset ${pos}; copying tail`);
...
const scanSubrecords = (
  ...
  let pos = 0;
  // `XXXX` carries the true size of the following subrecord (its own uint16
  // size field is then 0). Such oversized subrecords are never patched — only
  // walked over so later boundaries stay correct.
  let oversizeNext = 0;

  while (pos + SUB_HEADER_SIZE <= recordBuf.length) {
    const sig = recordBuf.toString('ascii', pos, pos + 4);
    const hadOversize = oversizeNext > 0;
    const subSize = hadOversize ? oversizeNext : recordBuf.readUInt16LE(pos + 4);
    oversizeNext = 0;
    const dataStart = pos + SUB_HEADER_SIZE;
    const end = dataStart + subSize;
    if (end > recordBuf.length) break;
    const key = sig.toUpperCase();

    if (sig === 'XXXX' && subSize === 4) {
      oversizeNext = recordBuf.readUInt32LE(dataStart);
    } else if (!hadOversize && patchedSigs.has(key)) {
      slots.push({
  ...
  // Preserve any unparsed tail so the rebuild never silently drops bytes.
  if (pos < recordBuf.length) {
    bounds.push({ start: pos, end: recordBuf.length, sig: '' });
  }
...
const rebuildRecordData = (
  ...
  for (const [position, { start, end, sig }] of bounds.entries()) {
    ...
    // Subrecord size is uint16; an oversized translation would make
    // writeUInt16LE throw and abort the whole export. Keep the original text.
    if (newBuf.length > 0xffff) {
      log.warn(
        `ESP patcher: record ${formIdHex} ${sig} translation is ${newBuf.length} bytes ` +
          `(uint16 max 65535); keeping original text`,
      );
      subChunks.push(recordBuf.subarray(start, end));
      continue;
    }
