...
export class EspReader {
  ...
  constructor(filePath: string, game: GameType = 'fo4') {
    ...
    this.parseHeader();
    log.info(
      `ESP: ${filePath} — localized=${this.info.isLocalized}, masters=[${this.info.masterFiles.join(', ')}]`,
    );
  }
  ...
  private walkRange(
    ...
    while (pos + RECORD_HEADER_SIZE <= end) {
      ...
      if (sig === 'GRUP') {
        ...
        // A corrupt size smaller than the header would stall the walk forever.
        if (groupSize < GRUP_HEADER_SIZE) break;
        const groupEnd = pos + groupSize;
        const groupLabelRaw = buf.readUInt32LE(pos + 8);
        const groupType = buf.readInt32LE(pos + 12);
        const nextDialogTopicFormId =
          groupType === 7
        ...
        this.walkRange(
          ...
          Math.min(groupEnd, end),
          rows,
          nextDialogTopicFormId,
        );
        pos = groupEnd;
      } else {
  ...
  private parseRecord(
    ...
    let pos = 0;
    // `XXXX` carries the true size of the following subrecord when it exceeds
    // the uint16 size field (which is then written as 0).
    let oversizeNext = 0;
    while (pos + SUBRECORD_HEADER_SIZE <= recordData.length) {
      const subSig = recordData.toString('ascii', pos, pos + 4);
      const subSize = oversizeNext > 0 ? oversizeNext : recordData.readUInt16LE(pos + 4);
      oversizeNext = 0;
      const dataStart = pos + SUBRECORD_HEADER_SIZE;
      const dataEnd = dataStart + subSize;
      if (dataEnd > recordData.length) break;

      if (subSig === 'XXXX' && subSize === 4) {
        oversizeNext = recordData.readUInt32LE(dataStart);
      } else if (subSig === 'EDID') {
