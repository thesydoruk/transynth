...
describe('scene timing helpers', () => {
  it('treats linear dialogue as safe and timers/packages as constrained', () => {
    ...
    expect(isTimingSensitiveAction(action({ actionType: 'timer', topicFormId: null, topicFormIds: [] }))).toBe(
      true,
    );
    expect(isTimingSensitiveAction(action({ actionType: 'package', topicFormId: null, topicFormIds: [] }))).toBe(
      true,
    );
