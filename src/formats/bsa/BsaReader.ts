...
export class BsaReader {
  ...
  extractEntry(entry: BsaFileEntry): Buffer {
    ...
    if (entry.isCompressed) {
      ...
      const compressedData = buf.subarray(entry.dataOffset + 4, entry.dataOffset + entry.dataSize);
      ...
      return Buffer.from(buf.subarray(entry.dataOffset, entry.dataOffset + entry.dataSize));
    }
  }
