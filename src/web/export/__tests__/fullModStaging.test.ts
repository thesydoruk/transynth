...
import { modImportLocalizeDir, modStorageRoot } from '../../../modStorage/paths';
...
describe('stageFullLocalizedMod', () => {
  ...
  it('overlays only database-allowlisted localized voice files', async () => {
    const extractRoot = path.join(modStorageRoot(), `_extracted_voice_stage_${Date.now()}`);
    tempArtifacts.push(extractRoot);
    fs.mkdirSync(extractRoot, { recursive: true });
    fs.writeFileSync(path.join(extractRoot, 'Demo.esp'), Buffer.from('TES4'));

    const originalGrunt = path.join(
      extractRoot,
      'Sound',
      'Voice',
      'Demo.esp',
    ...
    fs.mkdirSync(path.dirname(originalGrunt), { recursive: true });
    fs.writeFileSync(originalGrunt, Buffer.from('original-grunt'));

    const localizeDir = modImportLocalizeDir(extractRoot, 'uk');
    const gruntDub = path.join(localizeDir, 'Sound', 'Voice', 'Demo.esp', 'NPC', '000219CF_1.fuz');
    const lineDub = path.join(localizeDir, 'Sound', 'Voice', 'Demo.esp', 'NPC', '00123456_1.fuz');
    fs.mkdirSync(path.dirname(gruntDub), { recursive: true });
    fs.writeFileSync(gruntDub, Buffer.from('dubbed-grunt'));
    fs.writeFileSync(lineDub, Buffer.from('dubbed-line'));
      ...
      makeDb([
        {
          ...
          translation: 'Привіт, мандрівнику.',
          edid: null,
        },
        {
    ...
    try {
      const ba2File = result.files.find((file) => file.name.endsWith('.ba2'));
      expect(ba2File).toBeDefined();
      const ba2Path = path.join(os.tmpdir(), `voice-stage-ba2-${Date.now()}.ba2`);
      fs.writeFileSync(ba2Path, ba2File!.data);
      const reader = new Ba2Reader(ba2Path);
      try {
        const grunt = reader.extractByName('Sound\\Voice\\Demo.esp\\NPC\\000219CF_1.fuz');
        const line = reader.extractByName('Sound\\Voice\\Demo.esp\\NPC\\00123456_1.fuz');
        expect(grunt?.toString()).toBe('original-grunt');
        expect(line?.toString()).toBe('dubbed-line');
      } finally {
        reader.close();
        fs.unlinkSync(ba2Path);
      }
