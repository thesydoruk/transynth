...
import { llmGenderDetectRoutes } from './routes/llmGenderDetect';
...
await llmGenderDetectRoutes(app, db);
...
try {
  ...
  if (!isLoopbackHost(HOST)) {
    log.warn(
      `HOST=${HOST}: no login. Anyone who can reach this port can import/delete mods, spend LLM/TTS, and run ops. Do not expose to the internet without a reverse proxy. See SECURITY.md.`,
    );
  }
} catch (err) {
