...
export const projectSettingsRoutes = async (app: FastifyInstance, db: Tx) => {
  ...
  app.get('/api/project-settings', async (_req, reply) => {
    ...
    return reply.send({
      ...settings,
      'llm.vllm_servers': redactVllmServerEntries(settings['llm.vllm_servers']),
    });
  });
  ...
  app.put<{
    ...
    if (key === 'llm.vllm_servers') {
      ...
      const incoming = normalizeVllmServerEntries(value);
      if (value.length > 0 && incoming.length === 0) {
        return reply.code(400).send({
          error: 'Each vLLM host must be an http(s) URL',
        });
      }
      const current = await getAllProjectSettings(db);
      const normalized = retainVllmApiKeys(incoming, current['llm.vllm_servers']);
      log.info(`Project setting updated: ${key} (${normalized.length} servers)`);
      await setProjectSetting(db, key, normalized);
      syncLlmPoolFromProjectSettings(await getAllProjectSettings(db));
      return reply.send({ key, value: redactVllmServerEntries(normalized) });
    }
