/**
 * Disco wav-stem rows persisted in `voice_clips`.
 */
...
export const buildDiscoVoiceClipRows = (
  ...
  voiceFiles.map((entry) => {
    ...
    return {
      wavStem,
      lineKey: entry.lineKey || discoVoiceFormidLower6(wavStem),
...
export const aggregateDiscoClipSpeakerCounts = (
  clips: ReadonlyArray<{ speakerKey: string; lineKey: string; recordId: number | null }>,
  ...
  for (const clip of clips) {
    ...
    if (translationAudio.has(voiceTranslationMapKey(clip.lineKey, 1))) {
      group.dubbedCount += 1;
    }
  }
  return groups;
