 ...
 */
import { extractDiscoQuoteSpans, hasDiscoNarrationOutsideQuotes } from '../text/lockitMarkup';
...
const discoAsrTokensMatch = (left: string, right: string): boolean => {
...
const hasInterstitialDiscoNarration = (text: string): boolean => {
  const spans = extractDiscoQuoteSpans(text);
...
const discoQuoteHypotheses = (source: string): QuoteHypothesis[] => {
  const spans = extractDiscoQuoteSpans(source);
