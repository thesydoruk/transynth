...
import type { SynthesizeModVoiceLineResult } from '../../../voice/synthesizeModVoiceLine';
import { loadVoiceProjectSettings } from '../../../voice/voiceProjectSettings';
...
import { resolveTtsBaseUrl, type TtsReferenceMode } from '../../../voice/voiceToolPaths';
...
const prepareDiscoLine = async (
  ...
  const found = await resolveDiscoClipEntryByFormid(db, opts.modId, extractRoot, opts.lineKey);
  ...
  if (!canSynthesizeVoiceLine(row.source, row.translation, row.edid, DISCO_VOICE_MARKUP)) {
    return {
      ok: false as const,
