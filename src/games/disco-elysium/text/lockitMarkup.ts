...
const extractDiscoUiBracketSpans = (text: string): DiscoMarkupSpan[] =>
  collectRegexSpans(text, 'ui_bracket', DISCO_UI_BRACKET_RE);

const countDiscoEmDashes = (text: string): number => text.match(DISCO_EM_DASH_RE)?.length ?? 0;
...
const trimExtraDiscoEmDashes = (source: string, translation: string): string => {
  const needed = countDiscoEmDashes(source);
  let out = translation;
...
const unwrapExtraDiscoTitleSingles = (source: string, translation: string): string => {
