...
} from '../../../web/export/pexDecompileService';
import { MOD_IMPORT_DEFAULT_SOURCE_LOCALE } from '../../../import/mod/localeHelpers';
import {
  collectMcmLocalesForModParallel,
  buildMcmImportRows,
  loadMcmKeyMetaForMod,
} from '../../../import/mod/mcmLocales';
import {
  buildInterfaceTranslateCsvRows,
  collectInterfaceTranslateLocales,
} from '../../../import/mod/interfaceTranslate';
import { collectPexStrings, buildPexCsvRows } from '../../../import/mod/pexStrings';
import { commitExtrasStop, extrasStopRequested } from '../../../import/mod/run/extrasStop';
import type { ModImportBatchWriter } from '../../../import/mod/run/batchWriter';
import type { ModImportRunContext } from '../../contract';

const trackImportBatch = (ctx: ModImportRunContext, results: ModImportBulkResult[]): void => {
  if (!ctx.pruneStaleImportData) return;
  trackModImportBulkResults(results, ctx.keptImportRecordKeys, ctx.keptImportStringIds);
};

export const importMcmStringRows = async (ctx: ModImportRunContext): Promise<void> => {
  const importModId = ctx.importModId;
  if (importModId == null) throw new Error('Import mod id missing');

  const mcmModDir = resolveModDirectoryFromPath(ctx.anchorPath);
  const mcmLocales = await collectMcmLocalesForModParallel(mcmModDir, ctx.anchorPath);
  ...
  const mcmRows = buildMcmImportRows(sourceMcmMap, loadMcmKeyMetaForMod(mcmModDir, ctx.anchorPath));
...
export const importInterfaceTranslateRows = async (ctx: ModImportRunContext): Promise<void> => {
  const importModId = ctx.importModId;
  if (importModId == null) throw new Error('Import mod id missing');

  const modDir = resolveModDirectoryFromPath(ctx.anchorPath);
  const locales = collectInterfaceTranslateLocales(modDir, ctx.anchorPath);
...
export const importPexStringRows = async (
  ...
  const pexMap = await collectPexStrings(ctx.anchorPath);
  if (pexMap.size === 0) {
