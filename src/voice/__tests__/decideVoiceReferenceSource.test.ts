...
describe('decideVoiceReferenceSource', () => {
  it('uses speaker mode only when the line is a suitable mid-length take', () => {
  ...
  it('keeps a short line as short even in speaker mode', () => {
    expect(decideVoiceReferenceSource('speaker', false)).toEqual({ kind: 'short' });
  });
  ...
  it('marks an unsuitable short clip as short, not a speaker swap', () => {
    const decision: VoiceReferenceSourceDecision = decideVoiceReferenceSource('line', false);
    expect(decision).toEqual({ kind: 'short' });
  });

  it('never substitutes a long original clip with a shorter speaker reference', () => {
    expect(decideVoiceReferenceSource('line', false, true)).toEqual({ kind: 'line' });
    expect(decideVoiceReferenceSource('speaker', false, true)).toEqual({ kind: 'line' });
  });
});
