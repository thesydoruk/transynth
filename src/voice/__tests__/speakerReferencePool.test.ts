...
describe('scoreReferencePcm', () => {
  ...
  it('scores the 8–12s band higher than a 5s clip', () => {
    expect(scoreReferencePcm(makeSpeechLike(10), SAMPLE_RATE)).toBeGreaterThan(
      scoreReferencePcm(makeSpeechLike(5), SAMPLE_RATE),
    );
  });
});
...
describe('scoreReferenceWav integration', () => {
  ...
  it('scores WAV files on disk', () => {
    ...
    writeTestWav(wavPath, makeSpeechLike(10));
...
describe('resolveSpeakerReferenceForSpeaker', () => {
  ...
  it('uses the current line without scanning siblings when it is already 8–12s', async () => {
    const speaker = 'AlexanderBrown';
    const preferred = makeEntry(tmpDir, speaker, '00002CBA', 1, makeSpeechLike(10));
    const sibling = makeEntry(tmpDir, speaker, '00002CBB', 1, makeSpeechLike(10));
  ...
  it('scans siblings and prefers an 8–12s clip over a short current line', async () => {
    const speaker = 'AlexanderBrown';
  ...
  it('picks the closest longer clip when no 8–12s sibling exists', async () => {
    const speaker = 'AlexanderBrown';
    const preferred = makeEntry(tmpDir, speaker, '00002CBA', 1, makeSpeechLike(5));
    const longer = makeEntry(tmpDir, speaker, '00002CBB', 1, makeSpeechLike(13.2));
    const longest = makeEntry(tmpDir, speaker, '00002CBC', 1, makeSpeechLike(13.8));
    const db = createMockDb() as unknown as import('../../db').Tx;
    ...
    const result = await resolveSpeakerReferenceForSpeaker({
      ...
      speakerKey: speaker,
      preferredEntry: preferred,
      getFallbackEntries: () => [longer, longest],
      packageDir: tmpDir,
      pluginRelPath: 'Data/Mod.esp',
    });
  ...
  it('prefers the 8–12s clip whose speaking rate is closer to 5 syl/s', async () => {
    ...
    const sibling = makeEntry(tmpDir, speaker, '00002CBB', 1, makeSpeechLike(10));
    const db = createMockDb() as unknown as import('../../db').Tx;
    const pace = (syllables: number) => Array.from({ length: syllables }, () => 'go').join(' ');
  ...
  it('scores an 8–12s speech clip as a usable reference', () => {
    const speechScore = scoreReferencePcm(makeSpeechLike(10), SAMPLE_RATE);
