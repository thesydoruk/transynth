 ...
 *   Phonetic grunt — Agh! / Hmm… / Ha! → skip (phonetic_vocalization)
...
export const voiceTtsSkipMessage = (reason: VoiceTtsSkipReason): string => {
  switch (reason) {
    ...
    case 'phonetic_vocalization':
      return 'Vocalization / non-lexical sound — not synthesizable speech';
  }
};
 ...
 * - leftover source or translation after strip is a grunt / filler / laugh
 *
...
export const detectVoiceTtsSkipReason = (
  ...
  const sourceSpeak = stripVoiceNonSpeechBlocks(source, markup);
  const targetSpeak = stripVoiceNonSpeechBlocks(target, markup);
  if (isPhoneticVocalizationLine(sourceSpeak) || isPhoneticVocalizationLine(targetSpeak)) {
    return 'phonetic_vocalization';
  }
  return null;
};

/**
 * Skip reason for editor UI. Same as {@link detectVoiceTtsSkipReason}, plus
 * `empty_after_strip` when a non-empty translation strips to nothing. An empty
 * translation is not a skip — the line just has nothing to synthesize yet.
 */
export const resolveVoiceLineSkipReason = (
  lineSource: string | null | undefined,
  translation: string,
  edid?: string | null,
  markup: VoiceTtsMarkupStyle = 'fallout',
  ...
  const target = normalizeLine(translation);
  if (target && !stripVoiceNonSpeechBlocks(target, markup)) return 'empty_after_strip';
  return null;
};
 ...
 * @example canSynthesizeVoiceLine('Agh!', 'Агх!') → false
 */
