...
- `tsx` використовується для прямого запуску TypeScript-файлів у development і
  CLI workflow.
- `concurrently` використовується в dev-orchestration scripts.
- `npm run knip` показує код, до якого ніщо не дотягується: файли, експорти й
  залежності без споживачів. Налаштований так, щоб бути дієвим, а не
  вичерпним: репозиторій описано одним пласким проєктом (worker тягне `src/`
  відносними шляхами, тож окремий workspace ламає граф), а невживані _типи_ не
  показуються — більшість із них іменує сигнатуру власної ж функції. Норма —
  чистий прогін; усе, що він друкує, або мертве, або означає, що в `knip.json`
  бракує точки входу.
...
← [Конфігурація](14-configuration.md) | [Головна](README.md) | **Далі: [Vortex sync →](16-vortex-sync.md)**
