...
- **Verify** відкриває модалку (`llm-verify`). Опції: автопідтвердити рядки,
  ...
  У Disco після відповіді моделі спрацьовує **markup guard**: втрачені лапки
  піднімають verdict до `incorrect`, інші мітки lockit — до `suspicious`.
...
У system prompt модель окремо інструктується **не змінювати `¤PH0¤`- і `¤FK0¤`-подібні токени**. Для Disco те саме стосується `¤IT¤` / `¤Q¤` / `¤TS¤` / `¤EM¤` (див. нижче).
...
Порожні метадані (`form_id`, `context`, `edid`, …) у JSON до моделі **не** потрапляють. RAG-приклади несуть лише тексти й локацію, без `similarity` / `match_method`. У Disco поле `field` — це gettext **msgctxt** (`Dialogue Text/…`, `Alternate1/…`, `*_EFFECT`), не Bethesda subrecord.

---

## Disco lockit

Рядки Final Cut несуть службову розмітку, яку переклад має **дзеркалити за кількістю**, навіть якщо вид лапок зміниться (`"…"` → `«…»` у чернетці моделі):

| Розмітка         | Приклад             | Маска LLM               | Після розмаскування |
| ---------------- | ------------------- | ----------------------- | ------------------- |
| Лапки репліки    | `"Okay."`           | `¤Q0¤Okay.¤Q0¤`         | ASCII `"…"`         |
| Курсив           | `*belong*`          | `¤IT0¤belong¤IT0¤`      | `*belong*`          |
| Одинарні / title | `'Scab Leader'`     | `¤TS0¤Scab Leader¤TS0¤` | `'Scab Leader'`     |
| Тире lockit      | `dignity -- to die` | `dignity ¤EM0¤ to die`  | `--`                |
| UI-дужки         | `[Leave.]`          | не маскуються як ключ   | дужки лишаються     |

Парні ключі мають з’явитися **двічі**. `--` — пауза, не обгортка `--виділення--`. `*курсив*` у DE — italics, не Fallout-ремарка `*sigh*`.

Після відповіді моделі pipeline відновлює форму:

- `«»` складаються до ASCII `"…"` (і при кожному збереженні в редакторі);
- `'Title'` / вкладені одинарні, які модель звела до внутрішніх `"…"`, повертаються;
- зайві лапки навколо імен знімаються; тире `—` / `–` мапляться назад на `--`, якщо source їх має;
- латинські `*Prefect*`, які модель розгорнула, знову обгортаються зірками.

Перед LLM і TTS зірчаста цензура lockit (`f****t`-родина, ~89 рядків) розгортається в повне слово, щоб спрацював глосарій і голос. Зірки курсиву `*belong*` не чіпаються.

Озвучка ріже змішані рядки за ASR — див. [Озвучка](09-voice.md#disco-що-саме-озвучується).
