...
- [Verify, skip-detect і стать](#verify-skip-detect-і-стать)
  - [Що блокує підтвердження](#що-блокує-підтвердження)
  - [Захист роду](#захист-роду)
...
- **Verify** відкриває модалку (`llm-verify`). Опції: автопідтвердити рядки
  без доведеного дефекту; автопідставити правки для підозрілих; чи брати вже
  підтверджені переклади. Результати лишаються в модалці, доки її не закриєте.
...
### Що блокує підтвердження

Аудитор оцінює тон і кальку, але його думка більше не тримає рядок поза
рев’ю. Підтвердження блокує лише дефект, який система довела сама
(`protected_token_mismatch`, `markup_broken`, `gender_leak`,
`corrupted_translation`, `full_translation_mismatch`) або вердикт `incorrect`.
Голий `suspicious` пишеться в `qa_issues` як `llm_review` і показується в
редакторі; рядок іде далі. Правка, яка повторює вже бачений текст, відкидається;
поради переписують щонайбільше п’ять разів; нове формулювання має виграти
порівняння з тим, що вже стоїть, — порівняння робить модель, а якщо воно не
відбулося, лишається чинний текст. Доведений дефект ліміту не має і на це
порівняння не йде: його правку приймають, коли детектор, що довів дефект,
більше не заперечує. `dryRun` на ендпоінті лише звітує вердикти, нічого не пише.

Дефект, який детектор може довести, лагодять **до** аудиту, а не після нього:
рядки батчу з витоком роду спершу йдуть у repair-прохід, виправлене
формулювання записується, і модель оцінює вже той текст, який буде
підтверджено. Один запуск verify доводить рядок від витоку до `reviewed`; раніше
на це йшло два запуски, і аудит витрачався на текст, про який уже було відомо,
що він хибний.

### Захист роду

Українська маркує рід у минулому часі та предикативних прикметниках, тож
репліку, яку говорить (або чує) гравець, треба сформулювати так, щоб вона
читалася за будь-якої статі. У промпті це написано; модель здебільшого так і
робить, а в решті випадків тихо ставить чоловічий рід. Детектор читає не лише
форму біля «я» чи «ти»: безпідметові речення й присудкові прикметники теж —
українська постійно опускає підмет. Він працює у двох точках, де хибний рядок
ще можна зупинити:

1. **Після перекладу.** Рядки з витоком роду йдуть до моделі окремим проходом
   (`gender-repair` у логах LLM). Три промпти, обрані на рядок:
   - **agreement** — стать учасника відома: змінити закінчення, більше нічого;
   - **speaker** — гравець говорить про себе: безособове, пасив на -но, іменник
     замість «я»;
   - **addressee** — NPC говорить до гравця: дію адресата перетворити на подію,
     присвійне або річ. Це більшість витоків.
     Гілки йдуть одна за одною, не паралельно. Два нейтралізаційні промпти
     просять до трьох перефраз в одному виклику, а детектор бере першу справді
     чисту: заміна чоловічого роду на жіночий на рядку гравця, «зробив/ла» чи
     ховання за «ви» не проходять ту саму перевірку. На 60 реальних рядках із
     витоком це підняло прохід із 55 до 59 чистих. Рядок, який не очистив жоден
     варіант, лишається як був.
2. **Під час verify.** Витік роду — доведений дефект. Рядок із витоком іде в
   той самий repair-прохід ще до того, як модель його оцінює, тож аудит бачить
   уже виправлене формулювання; рядок, який прохід не зміг полагодити, блокує
   підтвердження. Вердикт `ok` на рядку з витоком знижують; запропоновану
   правку, яка сама вносить рід, відкидають. Вгадана стать оповідача (евристика
   чи LLM) більше не є доказом — лише ручне рішення тримає наративний рядок.

Те, що пройшло обидві точки, все одно спливе як QA `gender_mismatch`. Захист
працює лише для цільової мови `uk`.

### Як виміряти зміну промпта

Роботу над промптом неможливо спростувати без числа, яке можна зрушити, тож
`npm run eval:quality` оцінює збережені переклади за тими перевірками, у яких
машина може бути впевнена: рід, який суперечить метаданим, англійська калька,
зламаний placeholder. Результат розбито за випадками (`player_speaks`,
`player_addressed`, `known_gender`, `unknown_gender`, `not_dialogue`):

```bash
npm run eval:quality -- --mod 122 --out before.json
# змінюємо промпт, перекладаємо наново
npm run eval:quality -- --mod 122 --baseline before.json
```

Другий запуск покаже частку дефектів у кожному кошику і зміну у відсоткових
пунктах. Падіння цієї частки — умова необхідна, але не достатня: рядок може
бути чистим за всіма пунктами і все одно звучати мертво.

Мовця й адресата визначають під час імпорту, тож виправлення в цій логіці
доходить лише до нових імпортів. `npm run dialog:refresh -- --mod 122`
переганяє саме цей крок проти плагіна на диску — значно дешевше за повторний
імпорт і не чіпає нічого іншого. Рядки, переклади, статуси перевірки та ручні
`gender_override` лишаються; перезаписуються тільки `detected_gender` і колонки
адресата. Приймає також `--game <id>`, `--all` і `--dry-run`.
...
Кожен LLM-батч несе в запиті **до 24 термінів глосарію**: ті, чия англійська
форма трапляється в source батчу, а далі до восьми найближчих за ембеддингом —
для формулювань, які точний збіг пропустив. Вектори термінів будуються раз на
процес, партіями, які embedding-сервер приймає; невдалу побудову пам’ятають
десять хвилин, а не повторюють на кожному батчі. Повний глосарій у prompt не
потрапляє ніколи.

Приклад того, що бачить модель:
...
3. Перекладайте через LLM лише `untranslated` рядки, не перетираючи наявні людські або TM-переклади.
4. Після кожного batch перевіряйте QA-помилки.
5. Перед релізом обов’язково пройдіться по всіх рядках зі статусом `auto`.
