...
**Dialogs** (Bethesda): ліворуч навігатор — одне дерево, у якому теми, гілки,
сцени й розмови лежать разом; вибір вузла задає його scope, і цей вибір
зберігається в URL. У транскрипті є Fill TM / Fill LLM, пошук, наступна
незавершена репліка (`N`), відтворення (`P`) і редагування (`Enter`) — повний
набір див. у [Гарячих клавішах](04-keyboard-shortcuts.md#dialogs).

Кожна репліка має бейдж роду (♂ ♀ ⚥ ?) біля імені мовця. Він же і контрол:
клацніть, щоб перевизначити визначений рід — далі він виграє скрізь: у промпті
до моделі, у перевірці QA і в експортованому тексті. Невідомий `?` обведено
кольором попередження, бо саме його варто виправити.

Під сфокусованою реплікою видно, що саме знайшла QA — слово, яке закріплює
хибний рід, англійський зворот, який лишився, — а не лише лічильник. Рядки,
підставлені з пам'яті перекладів або моделлю, мають маленький тег `TM` чи `AI`,
а контекст, збережений із записом, друкується під джерелом.
...
- **llm_review**: зауваження verify (калька, тон, регістр), яке не блокує підтвердження
...
Допустимі значення статусів описані в `src/web/data/statusMachine.ts`. Автоматичні
