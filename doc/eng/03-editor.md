...
**Dialogs** (Bethesda): the left navigator is one tree holding topics, branches,
scenes and conversations together — selecting a node picks its scope with it,
and the choice lives in the URL. The transcript supports Fill TM / Fill LLM,
find, next unfinished line (`N`), play (`P`), and inline edit (`Enter`); see
[Keyboard shortcuts](04-keyboard-shortcuts.md#dialogs) for the full set.

Each speaker turn carries a gender badge (♂ ♀ ⚥ ?) next to the speaker's name.
It is also the control: click it to override the detected gender, which then
wins everywhere downstream — the LLM prompt, the QA check, and the exported
text. An unresolved `?` is outlined in the warning colour, because that is the
one worth correcting.

The focused line shows what QA found underneath it — the word that commits to
the wrong gender, the English phrase that survived — rather than only a count.
Lines filled from translation memory or by the model carry a small `TM` or `AI`
tag, and any context stored with the record is printed under the source.
...
- **llm_review**: a verify advisory (calque, tone, register) that does not block approval
...
Valid status values are defined in `src/web/data/statusMachine.ts`. Automated pipelines
